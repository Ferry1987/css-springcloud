...
@Configuration
public class CssRestTemplateApp {
    private Logger logger = LoggerFactory.getLogger(CssRestTemplateApp.class);
    @Bean
    public RestTemplate restTemplate() {
       final RestTemplate template = new RestTemplate(httpRequestFactory());
        logger.info("开始增加拦截 CssHttpRequestInterceptor");
        template.getInterceptors().add(new CssHttpRequestInterceptor());
        logger.info("拦截器增加结束");
        return template;
    }
